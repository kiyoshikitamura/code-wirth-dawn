...
- **クライアント側ガードには getSession を使用**: `useAuthGuard` のようなフロントエンド用の遷移ガードは、通信が発生しない `supabase.auth.getSession()` でローカルセッションを検証する方式にする。実データアクセスは各サーバーサイドAPIで `getUser(token)` により厳格に検証されているため、クライアント側で毎回ネットワークを介した `getUser()` を呼ぶ必要はなく、オーバーヘッドとバグの元にしかならない。
