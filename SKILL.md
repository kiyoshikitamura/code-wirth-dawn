...
- **データ更新ボタン追加時のSPA体験の維持**:
  - ダッシュボードなどで最新データを再取得する「リロード」機能を設ける際、単純に `window.location.reload()` でページ全体を強制リロードするのではなく、データフェッチロジックを `useCallback` などでコンポーネントルートに抽出し、画面遷移や瞬きを発生させずにデータのみをAPIからバックグラウンドで再取得（SPAライクな部分更新）する仕組みにすることで、運営の使い勝手とビジュアル体験を損なわない設計とすること。