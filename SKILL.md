...
- **ビジュアルフローエディタの自前実装**: 外部ライブラリ（@xyflow/react ≒ 60KB gzipped）を導入せず、SVG+CSS+React でカスタム実装。制限されたフロー構造（ノード20/分岐1段階/2択まで）であればカスタム実装で十分であり、バンドルサイズ増加を回避。
...
- **タッチイベント設計**: ノード移動はロングタップ（200ms遅延）→ドラッグで開始。通常タップはノード編集ボトムシートの表示に使用。ピンチズームは2点タッチの距離変化で viewport.zoom を更新（0.5x〜2.0x）。
