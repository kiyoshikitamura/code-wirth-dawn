...
- **非同期セッション確立のタイムラグ**: Google OAuth認証直後、SDKが認可コード（`code`）をセッションに交換し `localStorage` へ永続化するまでには微小な遅延がある。`useAuthGuard` などの遷移先ガードでは、初回の `getUser()` 失敗からリトライまでの待機時間を短縮しすぎず（1500ms以上を推奨）、タイムラグを適切に吸収する設計にすること。
