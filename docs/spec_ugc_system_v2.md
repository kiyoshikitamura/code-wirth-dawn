...
| メソッド | パス | 認証 | 概要 |
|---------|------|------|------|
| `POST` | `/api/ugc/v2/import` | JWT | テンプレートインポート（パース → draft保存） |
| `POST` | `/api/ugc/v2/validate` | JWT | テンプレートバリデーション（ドライラン） |
| `GET` | `/api/ugc/v2/template?type=&format=` | なし | テンプレートファイルのダウンロード（JSON/MD）。UGC_ENABLEDフラグに依存しない |
| `POST` | `/api/ugc/v2/publish` | JWT | 公開申請（draft → pending_review） |
| `POST` | `/api/ugc/v2/archive` | JWT | 公開取り下げ（published/pending → draft） |
| `GET` / `DELETE` | `/api/ugc/v2/list` | JWT | マイ作品一覧 / ドラフト削除 |
| `POST` | `/api/ugc/v2/test-complete` | JWT | テストプレイ完了記録（`tested_at` を更新） |
| `POST` | `/api/ugc/v2/asset/upload` | JWT | 画像・音声ファイルのアップロード |
| `GET` | `/api/ugc/v2/search` | なし | 公開クエストの検索 |
| `POST` | `/api/ugc/v2/calculate` | なし | バランス計算ツール（TP/NP/PB） |
...
```json
{
  "url": "ugc://images/scenarios/..."  // または "https://..." 形式の絶対URL
}
```

**処理**:
1. JWT認証
2. リクエストされたアセットURLからバケット名（`ugc-images` / `ugc-audio`）およびファイル名を抽出（`ugc://` 形式および `https://` 形式の両方に対応）
...
公開申請の前に、クリエイター自身が現在のキャラクターとデッキでクエストをプレイし、最終ノード（成功または失敗）に到達する必要がある。

#### テストプレイの仕様

- テストプレイ中のVitality消費、敗北ペナルティは発生しない
- テストプレイ中の報酬付与（ゴールド・EXP・アイテム）は発生しない
- テストプレイ中の名声変化、経過日数は発生しない
- テストプレイ結果画面では上記の値はすべて0/nullで表示される
- テストプレイ結果画面のボタンは「クリエイターズ工房に戻る」で `/workshop` に遷移

#### テストプレイ完了の記録

- 成功・敗北を問わず、最終ノードに到達した時点でテストプレイ完了とする
- ギブアップ、バトル中の撤退ではテストプレイ完了にならない
- 完了時に `POST /api/ugc/v2/test-complete` を呼び出し、`ugc_scenarios.tested_at` にタイムスタンプを記録
- クライアント側でも `localStorage` に記録（即時参照用）
...
| 認証 | JWT必須 |
| 権限 | `creator_id === user.id` のみ |
| 条件 | `status === 'draft'` のみ |
| 処理 | `tested_at = now()` を UPDATE |

- テンプレートの再インポート（内容変更）時は `tested_at` がNULLに戻る

#### マイ作品パネルでの表示

- draft作品に「▶ テストプレイ」ボタンを表示（`/quest/{id}?test_play=true` に遷移）
- `tested_at` が null → 「⚠ 要テストプレイ」バッジ（amber）
- `tested_at` が設定済み → 「✅ テスト済み」バッジ（emerald）
- 「申請」ボタンは `tested_at` が null の場合 disabled
...
|------|--------|-----------|---------------|----------|
| Free | 1 | 5 | エネミー5 / アイテム5 / スキル5 / NPC5 | 10MB |
| Basic | 5 | 10 | 各20 | 50MB |
| Premium | 30 | 50 | 無制限 | 200MB |

### 9.2 Tier別レートリミット（1日あたり）

| Tier | 公開申請 | 保存 | インポート |
|------|---------|------|----------|
| Free | 1回 | 5回 | 10回 |
| Basic | 3回 | 15回 | 30回 |
| Premium | 10回 | 50回 | 100回 |

枠チェック・レートリミットは各API操作時にサーバーサイドで実施。超過時は `403 Forbidden` または `429 Too Many Requests`。

### 9.3 ゴールドによる枠追加

ユーザーはゲーム内通貨（ゴールド）を消費して、Tier上限を超える追加枠を購入できる。

| 拡張項目 | 1回あたりのコスト | 追加量 | 追加上限 | 永続性 |
|---------|----------------|--------|---------|-------|
| 下書き枠 | 2,000G | +1枠 | Tier上限の2倍まで | 恒久 |
| 公開枠 | 10,000G | +1枠 | Tier上限の2倍まで | 恒久 |
| インポート回数 | 2,000G | +5回/日 | 制限なし | 当日限り（翌日UTCリセット） |

**DBカラム**: `user_profiles` テーブルに以下のカラムを追加:
- `ugc_extra_drafts` (INT, DEFAULT 0): ゴールド購入による追加下書き枠数
- `ugc_extra_published` (INT, DEFAULT 0): ゴールド購入による追加公開枠数
- `ugc_extra_daily_import` (INT, DEFAULT 0): 当日追加インポート回数（daily cronでリセット）

**枠チェックの計算**: `有効枠 = Tier基本枠 + 追加枠`
...
### 9.4 Workshop ステータスパネル

Workshop ページ上部に常時表示するステータスパネル:
- **レイアウト構成**:
  - **上段（1行目）**: 左寄せに **Tierバッジ**、右寄せに **所持ゴールド表示**、**「枠追加」ボタン**、**「更新」ボタン** を横並びに配置（左右両端に寄せる）。
  - **下段（2・3行目）**: **使用状況表示**（下書き枠、公開枠、1日あたりのインポート回数、および素材容量の4つの項目）を2カラム×2行のグリッド配置で表示。
- **Tierバッジ**: Free（灰色） / Basic（青色 ⚡） / Premium（金色 👑）
- **使用状況表示**: モバイル表示時の改行崩れを防止するため、進捗バー（インジケーター）なしのコンパクトなバッジ形式（項目名 ＋ 数値のみの横並び）として表示する。
- **所持ゴールド表示**: 🪙 所持ゴールドを表示。
- **「🪙 枠追加」ボタン**: ゴールド枠追加モーダルを起動。ゴールド不足時には、宿屋（`/inn`）のアカウント設定（ゴールド購入）へ遷移するリンクを提供。
- **UX最適化**:
  - 読み込み中はスケルトン（スピナー）を表示。
  - タブ切り替え時の不要な再読み込み（再fetch）を防止するキャッシュ制御。
  - 使用率80%以上で警告色（amber）、100%で赤色表示。
  - **リアルタイム状態同期**: アセットアップローダー、作品一覧、インポーター、簡易作成の各パネルでアセットのアップロード/削除、下書きのインポート/作成/削除、公開申請/取り下げ等の成功時に `ugc-status-updated` イベントをディスパッチし、ステータスパネル側がこれを検知して自動で再取得（サイレントアップデート）して容量や残枠の数値を動的に同期する。
...
### 11.1 クリエイターズ工房（`/workshop`）

ログイン済みユーザーがアクセス可能（拠点制限なし）。
...
| インポート | テンプレートのテキスト貼り付け/ファイルアップロード + リアルタイムバリデーション |
| テンプレート | テンプレートファイルのダウンロード + AIプロンプトガイド |
| バランス計算 | TP/NP/PBの自動計算ツール |
| 簡易作成 | 4ステップウィザードによるクエスト作成 |

**インポートタブ**: テキスト貼り付けエリア（フォントサイズ9px、モバイル最適化） / クリアボタン / リアルタイムバリデーション結果（500msデバウンス） / バリデーションのみ・インポートボタン / サーバーエラー詳細表示

**マイ作品タブ**:
- **ステータス表示**: ステータスバッジ（draft/pending/published/rejected）/ テスト済み・要テストプレイバッジ / 却下理由表示。
- **アクションボタン**: ▶テストプレイボタン（モバイルでは「テスト」と短縮表示）、申請ボタン（tested_at必須）、削除・取り下げボタン。
- **モバイルレイアウト最適化**:
  - 操作ボタン群を `flex-wrap` と `whitespace-nowrap` で制御し、折り返されてもボタンの文字が途切れないように調整。
  - モバイルの幅に合わせて、メタ情報とボタンを縦積みの構成に変更。

**テンプレートタブ**: テンプレート種別×形式の選択 / ダウンロードボタン / AIプロンプトガイドモーダル / 記法クイックリファレンス
