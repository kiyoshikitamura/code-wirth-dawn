...
Workshop ページ上部に常時表示するコンパクトなステータスバー:
- Tierバッジ（Free / Basic⚡ / Premium👑）
- 使用状況プログレスバー（下書き・公開・日次インポート）
- 所持ゴールド表示
- 「🪙 枠追加」ボタン → ゴールド枠追加モーダル
...
**マイ作品タブ**: ステータスバッジ（draft/pending/published/rejected）/ テスト済み・要テストプレイバッジ / ▶テストプレイボタン / 申請ボタン（tested_at必須）/ 却下理由表示 / 削除・取り下げボタン
