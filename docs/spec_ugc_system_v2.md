...
テンプレートアップロード（MD/JSON）をメインとしつつ、モバイルユーザーがUI操作のみでクエストを作成できる「簡易クエストビルダー」をクリエイターズ工房に新設する。ビジュアルフローエディタ（SVG自前実装）を採用し、初心者でもクエスト全体のフローを視覚的に把握しながら作成できる。
...
- **ビジュアルフローエディタ**: SVG+CSS+React によるカスタム実装（外部ライブラリ不使用）
...
- **フローキャンバス**: SVGベースのノード配置・接続線描画エリア。ピンチズーム（0.5x〜2.0x）、パン操作対応。右下にミニマップ
- **ノード編集**: タップでボトムシート表示（画面60%高、フルスクリーンまで拡張可）
- **基本情報パネル**: ヘッダーボタンでサイドパネルスライドイン（クエスト名、報酬設定含む）
- **ツールバー**: 画面下部固定。5種のノードタイプアイコンボタン
