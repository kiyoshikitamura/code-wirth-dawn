...
<!-- v13.6 (2026-07-04): 最終継承＆英霊雇用割引仕様を統合 -->
...
| ゴールド継承率 | 50% (上限なし) | **100% (全額・上限なし)** | **100% (全額・上限なし)** |
| スキル継承率 | **100% (全自動継承)** | **100% (全自動継承)** | **100% (全自動継承)** |
| アイテム継承枠 | 最大 10個 | 最大 30個 | 最大 50個 |
| 英霊 (Heroic) 登録数 | 最大 1体 | 最大 3体 | 最大 10体 |
| 自身の英霊の雇用保証 | ❌ 不可 (全体ランダム枠のみ) | ✅ **可能 (専用リスト雇用)** | ✅ **可能 (専用リスト雇用)** |
| 英霊雇用割引率 | なし (0%) | なし (0%) | 🔥 **50% OFF (半額)** |
| LP (レガシー) 倍率 | 1.0倍 (等倍) | **1.2倍** | **1.5倍** |
...
引退時、自身が英霊の登録上限に達している場合は、**最も古い自身の英霊を削除（押し出し）した上で新規登録を行うFIFO（First In, First Out）方式**を採用する。

| Tier | 英霊登録上限 | FIFO条件 |
|---|---|---|
| `free` | 最大 1体 | 上限到達時に最古の英霊を削除 |
| `basic` | 最大 3体 | 上限到達時に最古の英霊を削除 |
| `premium` | 最大 10体 | 上限到達時に最古の英霊を削除 |

実装箇所: `lifeCycleService.ts` (`handleCharacterDeath` 内部)

> [!NOTE]
> Free Tier (`'free'`) のユーザーについても最低1体の英霊登録枠が保証されるよう拡張され、次の世代での英霊雇用の魅力（基本体験）を感じられる導線にアップデートされました。
