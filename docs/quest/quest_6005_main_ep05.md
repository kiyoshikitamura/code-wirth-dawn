...
| **ゲストNPC** | ガウェイン（パーティ加入あり / **本クエストで戦死・永久離脱**） |
| **クエスト完了後の拠点移動** | → 夜刀神国（辺境の関所） |
| **ノード数** | 16ノード（うち選択肢2件） |
...
**CSV記載形式:**
```
Exp:200|Gold:600|Rep:15|Item:501|Justice:10
```
...
**テキスト:**
```
突如として大地の揺れる音が響く。地平線を埋め尽くすほどの砂埃を上げて、王国軍の本隊が拠点への全面侵攻を開始したのだ。
```
**次ノード:** `{type:text, bg:bg_bandit_camp}` (auto-advance)
**params:**
```json
...
**テキスト:**
```
怒涛の如く押し寄せる王国兵の群れ。彼らの目には熱狂的な『大義』が宿っており、命の惜しさなど微塵も感じられない。まずは先陣を切り抜けろ！
```
...
**テキスト:**
```
矢衾の中から、重装装甲に身を包んだ王国の精鋭部隊が現れた。彼らの標的は、最前線で踏み留まっているガウェインだ。
```
**次ノード:** `{type:text, bg:bg_bandit_camp}` (auto-advance)
**params:**
```json
...
**テキスト:**
```
「無理だ、多すぎる！……新入り、お前はまだ死ぬべきではない。ここは俺が引き受ける、お前だけでも逃げろ！」
```
**次ノード:** `{type:text, bg:bg_bandit_camp, speaker_image_url:/images/npcs/npc_guest_gawain.png}` (auto-advance)
**params:**
```json
