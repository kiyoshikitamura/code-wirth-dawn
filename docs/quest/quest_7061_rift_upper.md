...
### ④ 行き倒れた冒険者ノード (地下2階)
*   **Node ID**: `7061_b2f_adventure`
*   **背景**: `bg_rift_upper_01`
*   **テキスト**:
    > 通路の隅に、力尽きてから随分と時間が経っていると思われる冒険者の遺体を見つけた。傍らには古い鞄が転がっている……。
*   **選択肢**:
    1.  `冒険者の鞄を見る（悪意+5、アライメント獲得トースト）` ➔ 遷移先: `7061_b2f_adventure_bag`
    2.  `そのまま黙祷を捧げる（正義+5、アライメント獲得トースト）` ➔ 遷移先: `7061_b2f_adventure_pray`

### ⑤ 冒険者鞄取得ノード / 黙祷ノード
*   **Node ID**: `7061_b2f_adventure_bag` (悪意+5獲得) / `7061_b2f_adventure_pray` (正義+5獲得)
*   **処理**: アライメント変動（`reputation` の `evil`/`justice` に対応）が実行され、アライメント名（「正義」「悪意」）がローカライズされた獲得通知トーストが画面上部に表示されます。
