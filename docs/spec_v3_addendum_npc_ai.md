...
## 10. NPC AI・バトルUIおよび見切れ防止スクロールバーの改修 (v4.4 2026-07-08 追加)

### 10.1 味方お供NPCスキルカットイン演出の最適化
- **デザイン分離**:
  - 味方お供NPC（将哉など）がスキルを発動した際、エネミー用の赤い警告カットインが誤表示される問題を修正し、プレイヤーと同様に画面下部にスキル名のみをポップアップする表示に最適化。
  - 演出カラーはプレイヤーの青（Sky/Cyan）に対し、味方お供NPCは**緑（Emerald/Green）**として色による敵味方の差別化を徹底。
- **同名お供NPCの敵味方判定分離（ステルスマーカー方式）**:
  - アリーナ等で敵味方に同名のお供（例: 将哉）が存在した際、主語の文字列マッチングだけでは敵の攻撃が味方のスキルカットイン（緑）として誤認識される問題が発生する。
  - これを解消するため、味方お供NPCのアクションログメッセージの先頭にのみ、非表示の**ゼロ幅スペース（`\u200B`）**をステルスマーカーとして埋め込む方式を採用。
  - UIの描画フックは先頭の `\u200B` の有無で100%確実に味方の攻撃であるかを判定し、パース時にマーカーを除去して描画する。

### 10.2 多段ヒットダメージポップアップの重なり防止
- **ランダムオフセット座標の付与**:
  - 『雷電の連鎖』などの3連撃メッセージ（`1撃目: ◯◯に 38 ダメージ` 等）が連続出力された際、ポップアップ座標が完全に一致しているため重なり合い、1つしかポップアップが見えなくなる問題を解消。
  - ポップアップ発生時に、個々に `offsetX = Math.floor(Math.random() * 40) - 20` (-20px 〜 20px) および `offsetY = Math.floor(Math.random() * 20) - 10` (-10px 〜 10px) のランダム位置ずれパラメータを付与。
  - レンダリング時に `transform: translate(offsetX, offsetY)` を適用し、バラバラと飛び出すような美しいビジュアルへと改善。

### 10.3 画面見切れ防止用カスタムスクロールバーの導入
- **縦方向（上下）の見切れ回避**:
  - モバイル端末や高さを狭めたPCブラウザで画面下部の「NEXTボタン」や手札が画面外に見切れるのを防ぐため、ルートコンテナから `no-scrollbar` を除去。
  - 代わりに `custom-scrollbar-vertical` を適用し、コンテンツがはみ出した環境でのみ、アンバー色の細身の縦スクロールバーを自動表示しスクロール可能に制御。