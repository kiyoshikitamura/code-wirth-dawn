...
**`random` グレード:**
1. `signature_deck` からランダムに1枚を候補に選出。
2. AP不足の場合 → 行動スキップ。
3. `used_this_turn` に含まれている場合 → 別の候補を再選出。
4. 有効なカードが見つからない場合 → 行動スキップ。

**`smart` グレード（英霊・特殊 NPC）:**
1. **絊急回復チェック**: プレイヤー or 味方の HP が 50% 以下​→手札の回復系カードを優先使用。
2. **AP温存判断**: 高コストスキル（AP≥ 5）があり APが不足する場合​→行動を「スキップ」し次ターンに向けAPを温存。
3. APを消費できる限り、高コスト順にカードを使用。
...
| 項目 | 実装状態 |
|---|---|
| Heroicの溜め行動 | ✅ **実装済み** (`evaluateWaitLogic()`) |
| Smart AIの戦略判定 | ✅ **実装済み** (`resolveNpcTurn()` AP順・高コスト順) |
| Medic の HP 監視 | ✅ **実装済み** (`tryEmergencyHeal()`) |
