...
- **上限到達時の処理 (手動選択 ＆ FIFOの一括上限適用)**:
  - 上限に達している場合（プランのダウングレード等で大幅に超過している場合を含む）、引退確認画面（`InheritanceModal`）にて「上書き削除する英霊」を1体手動で選択できる。
  - 手動選択された英霊が削除された後、依然として登録総数がプランの上限値（新規枠確保のため `上限 - 1` 体）を超過している場合は、残りの古い英霊から最古のものを順に自動でFIFO削除する。これにより、上書き選択の有無にかかわらず、最終登録数が現在のプラン上限（Freeなら1体）へ確実に収束するように制限を適用する。
...
| クエスト完了履歴 | 部分的 | メインシナリオ (`main_ep*`) のクリア記録のみ永続保持。それ以外は削除。※ただし、「継承クエスト『英霊の祝福』(ID: 6022)」のクリア履歴のみ、世代交代ごとに再受注できるように特別にクリアリセットされる。 |
...
- **反映**: キャラクタメイク画面（`title/page.tsx`）にてBP分配UIを表示し、`/api/profile/init` 時に割り振り数を送信。サーバー側でLP上限値を検証した上でステータスに適用。**なお、BPが余っている状態での作成完了・次への進行は禁止され、未割り振りBPが残っている場合は進行ガードとして `alert('BPが残っています。')` が表示される。**

---

## 5. 英霊の雇用保証 (My Heroic Spirits)
- **酒場「英霊の間」ボードの最大表示制限とランダム回転**:
  - 酒場（`TavernModal`）の「英霊の間」タブに表示される英霊数は最大 **1体** に制限される。
  - この1体は固定表示化を避けるため、ページリロードや酒場再入場時にデータベース内の全登録英霊からランダムに1体が抽出される（クエリ前に件数を取得し乱数offsetを range で指定する）。
  - 一般ユーザー（Free）はこの1体のみ雇用契約可能。
- **「英霊の記録」からの直接雇用および酒場（Tavern）へのボタン統合**:
  - 酒場の「英霊の間」タブのヘッダーに **「英霊の記録から雇う」** ボタンを追加。Basic/Premiumのサブスクリプション加入者は、ここから自分の全英霊リスト（最大3体/10体）を開き、任意の英霊を確実に指定して雇用できる。
  - Freeプランのユーザーには、このボタンの右下に **「Basic/Premium専用」** の警告ラベルが表示され、モーダル内の雇用ボタンも非活性（disabled）となりボタンテキストが「Basic/Premium専用」に変化する。
- **Premium特典**:
  - `subscription_tier === 'premium'` のユーザーは、自身の英霊の雇用費用（`contract_fee`）が **50% OFF（半額）** となる（酒場「英霊の間」のランダム表示枠と「英霊の記録」から雇う画面の双方に一貫して適用される）。
