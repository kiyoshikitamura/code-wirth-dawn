...
| **Hard** | 10戦 | 30種類 | プレイヤーレベル × 50 G | アイテム2点 ＆ スキル2点 |
...
2. **システム通知（個別化）**: `notifications` テーブルに、「コロシアムランキング報酬」として各入賞部門ごとに個別の未読通知をインサートする（空席の場合はスキップ。合算せずに部門ごとにレコードを分けることで、フロントエンドでの個別ポップアップ表示を可能にする）。
...
5. **履歴テーブルの30日保管と自動削除**:
   - 履歴テーブル `colosseum_ranking_history` の data は、集計日時から **1ヶ月間（30日間）保存**され、Cron実行時にそれ以上古い過去レコードは自動クリーンアップされる。
6. **リセット開始時間ガード**: 誤って古い集計データを消去しないよう、現在日時が **JST 2026-06-19 12:00:00 (JST)** 以降の場合のみ、上記の削除クエリ（リセット）を実行するガード条件を設ける。

### 6.4 ログイン時のお祝いポップアップ（個別受け取りダイアログ）
- **動作**: プレイヤーが拠点（宿屋）をロードした際、`/api/init-page` 経由で自身の未読のランキング報酬通知（メッセージが `🏆 コロシアムランキング報酬獲得！` で始まるもの）をロードする。
- **個別（合算なし）の連続表示**: 未読通知が存在する場合、それらをフロントエンド側のキューに格納し、1部門ずつ順番に `ConfirmDialog` をポップアップ表示する。
- **受け取りSE再生**: ダイアログ表示時にお祝いSE (`se_item_get`) を再生する。
- **既読化と同期**: ユーザーがダイアログの「受け取る」を押した際、非同期で対象の通知を `read = true`（既読）に更新し、所持ゴールドやインベントリデータを即座に画面へ再フェッチして最新状態に同期する。これにより、再度ロードした際には表示されないように制御する。
...
## 11. システム最適化と同期堅牢化 (v28.2)

### 11.1 クエスト進行状況の堅牢な初期化 (Resume Robustness)
- ユーザーがコロシアム等のクエストを開始した際、サーバー上の `current_quest_state` を `null` にリセットする。
- クライアント側（`/quest/[id]/page.tsx`）初期表示時、サーバーの `current_quest_state` が `null` の場合は、ローカルストレージ（Zustand永続化）のキャッシュ状態に関わらず、必ず `startQuest` を呼び出してローカル状態をリセットする。これにより、古い進行状態がサーバーを上書きして2戦目から始まる等のキャッシュ同期バグを完全に防止する。

### 11.2 Zustand セレクタによるパフォーマンス最適化 (Zustand Selectors)
- `BattleView.tsx` などの大規模コンポーネントにおいて、Zustandの `useGameStore()` からオブジェクトを丸ごと展開して取得するのを禁止し、必要なステートのみを個別に切り出すセレクタ形式（`state => state.battleState` 等）で取得する。
- これにより、毎文字タイプライター更新などの微細なストア更新による `BattleView` コンポーネント全体の不要な強制再レンダリングを防ぎ、バトルのパフォーマンスとUIの応答性を劇的に向上させる。
