...
## 15. チュートリアル終了後ナビゲーションの完全一本道・非アクション化（Version 4.0 追記）
...
1. **再設計の背景と目的**:
   ...
   - 本設計では、これらの「施設アクションとの連動」を完全に廃止し、**「純粋な施設説明ツアー（一本道）」**として再構築する。
   - ユーザーは各施設がどのような場所かをバナーテキストで順番に確認し、該当するボタンをタップすることで同期的にツアーを進行させ、最後にワールドマップへ進んで終了する。
...
2. **ツアーの進行制御**:
   ...
   - 推奨ボタンがタップされた際、実際のダイアログやモーダルを開く処理は実行せず、**単にツアーのステップを進める（`onboardingTourStep` をインクリメントする）のみ**とする。これにより、ロード時間や画面切り替えのチラつきを完全に排除する。
   - 例: ステップ 1（宿屋/酒場）で「宿屋/酒場」施設ボタンをタップすると、宿屋/酒場モーダルを開くことなく、バナーテキストがステップ 2（ギルド）に更新され、ギルドボタンがハイライトされる。

3. **各ステップの文言とアピール対象**:
   - **ステップ 1: 宿屋/酒場**
     - 推奨ボタン: `inn` (宿屋/酒場) 施設ボタン
     - バナーテキスト: 「宿屋/酒場では仲間を探すことができます。」 (または「💡 旅の目的: 宿屋/酒場では仲間を探すことができます。」)
   - **ステップ 2: ギルド**
     - 推奨ボタン: `guild` (ギルド) 施設ボタン
     - バナーテキスト: 「ギルドでは様々なクエストを受けることができます。」 (または「💡 旅の目的: ギルドでは様々なクエストを受けることができます。」)
   - **ステップ 3: 道具屋**
     - 推奨ボタン: `shop` (道具屋) 施設ボタン
     - バナーテキスト: 「道具屋では様々なアイテムを購入することができます。」
   - **ステップ 4: 魔術学院**
     - 推奨ボタン: `magicAcademy` (魔術学院) 施設ボタン
     - バナーテキスト: 「魔術学院では魔導の契約やデッキの確認ができます。」
   - **ステップ 5: 設定/課金**
     - 推奨ボタン: ヘッダーの `設定 (Cog)` または `課金 (CreditCard)` ボタン
     - バナーテキスト: 「設定や課金では、音量の調整や特別なパッケージの購入ができます。」
   - **ステップ 6: ワールドマップ**
     ...
     - バナーテキスト: 「「出発する」を押して、ワールドマップから冒険に旅立ちましょう！」
     - 動作: Compass ボタンをタップした際、ツアー状態を `'completed'` に更新するのみとし、ワールドマップ画面（`/world-map`）へは遷移せず、そのまま拠点（`/inn`）に留まる。
...
## 23. プロモ待機ガードの完全持続とリザルト画面遷移ローディングの前面表示化仕様 (Version 4.9.1 追記)

1. **プロモ表示中における操作ガードカバーの持続と z-index 設計**:
   - プロモモーダル起動待ち、および表示されている間、宿屋背後画面の誤タップを防ぐガードカバー（`z-[300]`）を有効にし、モーダルが実際に閉じられた際（`handleCloseGuestRegisterPromo` 等）に初めて `isPromoPending` を `false` にしてガードを解除するよう制御する。
   - ガードカバー表示中もモーダル自体は正常にクリック可能とするため、プロモモーダル（`GuestRegisterPromoModal.tsx`, `StarterPackPromoModal.tsx`）の最外枠 z-index を `z-55` から **`z-[1000]`** に引き上げ、ガードカバー（`z-[300]`）より手前に配置する。
2. **遷移前非同期処理の await の徹底による描画フリーズ解消**:
   - `QuestResultModal` を閉じて宿屋へ遷移する際、親（`/quest/[id]/page.tsx`）の `onClose` 内で行われる非同期処理（`fetchUserProfile()`、`fetchInventory()`）を **`await` を付与して同期的に完了を待つ**。
   - これにより、Next.js のルーター遷移処理によるメインスレッドの占有・ブロックが発生する前に、ボタン自体のローディング状態（`isClosing === true` に伴う `[スピナー] 読み込み中…`）が画面に確実にペイントされるようにする。
3. **リザルト画面ローディングオーバーレイの前面化**:
   - `QuestResultModal` が `isClosing === true` の時に表示される全体ローディングオーバーレイ（`bg-slate-950/90`）の z-index を `z-[200]` から **`z-[2000]`** に引き上げる。
   - これにより、ボタンが押された瞬間、モーダルカード（`z-10`）および背面にあるシナリオエンジンの古いスピナー（「画面を切り替え中...」）などを完全に覆い隠し、透過残像による見栄えの悪さを根絶する。

---

## 24. ツアー自動開始のレースコンディション防止およびState駆動の描画保証付き遷移仕様 (Version 4.9.2 追記)

1. **ツアー自動開始の二重ガードによるレースコンディションの根絶**:
   - `useInnPageState.ts` において、マウント時のツアー自動開始判定 `useInnPageState.ts` の `useEffect` 内で、自動的にステップ '1' に初期化する条件に、`isPromoPending` 等のステートだけでなく、**`sessionStorage.getItem('wirth_dawn_quest_just_cleared') === 'true'` も直接チェックする**条件を追加する。
   - これにより、マウント直後のハイドレーション時やステート更新の遅延ラグが発生しても、セッション内に帰還フラグが存在する限り、ツアーが誤ってプロモモーダルの裏で同時開始されるのを完全に保留する。
2. **`isPromoPending` の初期化ハイドレーションミスマッチ解消**:
   - `isPromoPending` の初期値を `false` に統一し、マウント時の `useEffect` で `sessionStorage` をチェックして `true` に同期させることで、SSRとCSRのハイドレーションミスマッチによるステート異常を防止する。
3. **「冒険を続ける」ボタン押下時の描画保証遷移（State駆動のローディング表示）**:
