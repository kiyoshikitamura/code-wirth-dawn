...
- **テストアカウントの除外 (v33.0)**: キャラクター名が「テスト」または「test」で始まるアカウントは、表示条件を満たしていても酒場リスト（アクティブ残影）には表示されない。
...
> **テストアカウントの除外 (v33.0)**: 対象のプレイヤー名が「テスト」または「test」で始まるテストアカウントである場合、雇用契約はサーバー側で拒否される（「テストアカウントは雇用できません」エラー）。
