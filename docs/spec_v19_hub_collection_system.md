...
- **役割分離**: 本仕様書で定義される「名声」「アライメント」ランキングは Hub（名もなき旅人の拠所）でのみ閲覧可能なソーシャル機能である。これに対し、通常拠点で挑戦可能な「コロシアム」の戦績（勝利数・最多連勝数）については、独立した `ranking_colosseum_cache` テーブルおよび専用 API (`/api/ranking/colosseum`) を用いて分離して集計される。
