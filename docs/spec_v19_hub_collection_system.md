...
### 7.1 コレクションAPIとデータベーススキーマ不一致の解消
- **エネミー情報のDB参照先カラム修正**: `enemies` テーブル内の `exp_reward` / `gold_reward` が、実際のテーブルスキーマ定義で `reward_exp` / `reward_gold` となっていたため、参照エラーを修正した。
- **リレーション未定義カラムの動的取得**: `enemies` テーブルには `drop_item_name` カラムが存在しなかったため、API側で `items` マスタから `drop_item_slug` をキーにメモリ上でルックアップ（動的紐付け）し解決する構造に変更した。

### 7.2 UI可読性（コントラスト）の改善
- 暗色テーマ（`#0d1a2e`）での視認性を確保するため、クエスト記録およびコレクション内の補助的な暗色文字（No.ナンバリングや未解放プレースホルダー `─────`）について、カラー設定を `text-gray-600` / `text-gray-700` からより明度の高い `text-slate-400` / `text-slate-500` に変更した。

### 7.3 名声ランキングが「データがありません」となる挙動
- データベース（`reputations` テーブル）に名声レコードが存在しない初期状態またはテスト環境において、名声ランキングが空（データがありません）と表示されるのは正常な仕様の動作である。

### 7.4 コロシアムランキングとの役割分離とキャッシュ設計（追加改修・v24.0）
- **役割分離**: 本仕様書で定義される「名声」「アライメント」ランキングは、Hub（名もなき旅人の拠所）に限らず通常拠点の宿屋からも閲覧可能なソーシャル機能である。これに対し、通常拠点で挑戦可能な「コロシアム」の戦績（勝利数・最多連勝数）については、独立した `ranking_colosseum_cache` テーブルおよび専用 API (`/api/ranking/colosseum`) を用いて分離して集計される。
- **未挑戦除外ルール**: 名声・アライメントランキングは全ユーザーを母集団とするが、コロシアムランキングでは「勝敗数（wins + losses）が0」の未挑戦ユーザーはランキングの集計・表示対象から完全に除外される。
- **集計上限**: 最大500位までのアクティブユーザーをキャッシュテーブルに保持し、ユーザー自身の現在順位はリアルタイムでクエリされ上位表示される。
