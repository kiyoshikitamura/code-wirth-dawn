Code: Wirth-Dawn Specification v11.2 (Unified & Revised based on actual implementation)
# Quest Structure & Scenario Engine

## 1. 概要 (Overview)
本仕様書は、クエストのデータ構造と出現ロジック、シナリオエンジンのフロー、およびインベントリ・裏切りメカニクスを統合定義する。

---

## 2. データ構造

### 2.1 scenarios テーブルとID管理ルール

```sql
CREATE TABLE scenarios (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  slug TEXT UNIQUE,
  title TEXT NOT NULL,
  quest_type TEXT DEFAULT 'normal',   -- 'normal' | 'special' | 'main'
  location_id UUID REFERENCES locations(id),
  difficulty INT DEFAULT 1,
  rewards JSONB,                       -- { exp: number, gold: number, items: [...], reputation: number, etc. }
  conditions JSONB,                    -- 受注条件（名声などの制限はここに一元化）
  days_success INT DEFAULT 1,
  days_failure INT DEFAULT 1,
  flow JSONB,                          -- シナリオノードの木構造
  short_description TEXT,              -- クエスト一覧ボード用テキスト (最大40文字程度)
  full_description TEXT,               -- 詳細モーダル用長文フレーバーテキスト
  status TEXT DEFAULT 'published'
);
```

**【IDの運用ルール】**
*   **公式（System）**: `5001`, `6001`, `7001` 等の整数ベースのIDを使用する。データベースの制約上UUIDカプセル化が必要な場合はスクリプトで決定的なUUIDへの変換を行うか、スキーマをTEXTに変更する運用を行うが、CSVなど人間の管理レイヤーにおいては「整数」を正当なIDとして取り扱う。
*   **UGC（User Generated）**: 将来のプレイヤー作成クエストなどは重複回避のため完全なUUID（`gen_random_uuid()`）を用い、公式クエストと安全に混在・共存させる。

### 2.2 items / inventory テーブルの構造

```sql
CREATE TABLE items (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  slug TEXT UNIQUE,
  name TEXT NOT NULL,
  type TEXT DEFAULT 'consumable',  -- 'consumable', 'trade_good', 'skill', 'key_item'
  base_price INT DEFAULT 100,
  effect_data JSONB,
  is_skill BOOLEAN DEFAULT FALSE,  -- trueの場合スキルとしてデッキ装備可能
  description TEXT
);

CREATE TABLE inventory (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id),
  item_id BIGINT REFERENCES items(id),
  quantity INT DEFAULT 1,
  is_equipped BOOLEAN DEFAULT FALSE,
  is_ugc BOOLEAN DEFAULT FALSE
);
```

---

## 3. クエスト出現ロジック (Appearance Logic) と受注条件

クエストの表示・受注制限は `conditions` JSONB に集約される（`api/location/quests` でフィルタリングを行う）。

**`conditions` / `requirements` で指定可能な主なキー:**
*   `min_reputation` (Number): プレイヤーの名声がこの値**以上**でなければ受注不可。（例: `80` = 英雄向け）
*   `max_reputation` (Number): プレイヤーの名声がこの値**以下**でなければ受注不可。（例: `-80` = 悪党向け）
*   `completed_quest` (String|Number): 前提となるクエストの `slug`（あるいはID）をクリア済みか。
*   `location_tags` (Array): 出現拠点フラグ。`'all'` または国別タグ（`loc_holy_empire`, `loc_marcund`, `loc_yatoshin`, `loc_haryu`）。
*   `min_align_order_pct` (Number): 個人の秩序率（Order⇔Chaos軸）がこの%以上で出現。（v26）
*   `min_align_chaos_pct` (Number): 個人の混沌率がこの%以上で出現。（v26）
*   `min_align_justice_pct` (Number): 個人の正義率（Justice⇔Evil軸）がこの%以上で出現。（v26）
*   `min_align_evil_pct` (Number): 個人の悪率がこの%以上で出現。（v26）
*   `min_world_alignment` (Object): 世界の対立軸割合が条件を満たすか。（v26）
    *   例: `{ "axis": "chaos", "min_pct": 40 }`
*   `alignment_and` (Array): 個人+世界のアライメントAND条件。全要素を同時に満たす必要がある。（v26）
    *   例: `[{ "scope": "personal", "axis": "evil", "min_pct": 60 }, { "scope": "world", "axis": "chaos", "min_pct": 40 }]`

> **⚠ 重要**: 過去存在した単独カラムとしての `min_reputation`, `max_reputation` 等は廃止（または無視）し、すべて `conditions` JSONB内で数値ベースで管理・判定する。
> **v26 変更**: アライメント条件は旧方式（`min_align_chaos`=絶対値）から新方式（`min_align_chaos_pct`=対立軸割合%）に移行。旧キーもフォールバックとして動作する。

### 3.1 `completed_quest` の照合ロジック (v11.3)
`completed_quest` の値は slug（例: `"main_ep01"`）または数値ID（例: `7013`）のどちらでも指定可能。
API側で `slugToIdMap` を構築し、slug → ID 変換後に `user_completed_quests.scenario_id` と照合する。

### 3.2 クリア済クエストの非表示ルール (v11.3)

| クエスト種別 | クリア済非表示 | 理由 |
|---|---|---|
| **normal** (汎用クエスト 7000番台) | 非表示にしない（リピート可能） | 同じクエストを繰り返し受注できる |
| **special** (スポット/イベント) | 現世代では非表示 | 継承後は再出現する |
| **special (main_ep*)** | 永久に非表示 | メインシナリオはアカウント通じて1回のみ |

### 3.3 難易度タイア (difficulty_tier) 分類 (v11.3)

`rec_level` 値に基づき、ギルドUIのタブに振り分ける：

| Tier | rec_level 範囲 | UI表示 |
|---|---|---|
| **Easy** | 1–3 | 初心者向け |
| **Normal** | 4–7 | 中級者向け |
| **Hard** | 8+ | 上級者向け |

### 3.4 クエストの表示件数制限とSpecialタブ仕様 (v11.4)

*   **Normalクエスト**: API側でシャッフル後、難易度（tier）ごとに上限件数を設け、対応するタブ（Easy / Normal / Hard）に表示する：
    | Tier | 最大表示件数 |
    |---|---|
    | Easy | 5件 |
    | Normal | 3件 |
    | Hard | 1件 |

*   **Specialクエスト (新仕様)**:
    *   表示件数上限およびシャッフル制限を完全に撤廃。
    *   出現・受注条件（名声、アライメント、前提クエストクリア状況、所持アイテム）をクリアしたすべてのSpecialクエスト（メインシナリオおよび狭間の迷宮を含む）を表示する。
    *   ギルド等の依頼掲示板UIにおいて、Easyの左隣に「Special」タブを新設し、初期ファーストビューでアクティブ表示する。他の難易度別タブには混同を防ぐためnormalクエストのみを表示し、重複カウントを排除する。

### 3.5 国別フィルタリング (v11.3)
normalクエストの `location_tags` は、現在拠点の `ruling_nation_id` を以下のマッピングで変換して照合する：

| ruling_nation_id | location_tag |
|---|---|
| `Roland` | `loc_holy_empire` |
| `Markand` | `loc_marcund` |
| `Yato` | `loc_yatoshin` |
| `Karyu` | `loc_haryu` |

---

## 4. シナリオノード構造 (flow JSONB)

`flow` カラムにはツリー構造のノードデータが格納される。

### 4.1 公式定義ノード（`type`）一覧

| Type | 機能・役割 |
|---|---|
| `text` | テキストの表示と文字送り。 |
| `choice` | 選択肢の提示。ユーザーの回答により `next` へ分岐する。 |
| `battle` | バトルの開始（`enemy_slug` またはグループIDを指定）。 |
| `shop` | クエスト中専用ショップを開く。 |
| `travel` | ワールドマップを開き、移動先を指定させる。 |
| `camp` | 野営地メニュー（HP回復、デッキ調整など）。 |
| `reward` | クエストの途中報酬（アイテム等）を付与する。 |
| `end` / `end_success` / `end_failure` | クエスト成功（または失敗）として完了処理へ移行。`end_success` / `end_failure` でtype自体に結果を含める方式と、`end` + `result: 'success'/'failure'` で明示する方式の両方をサポート。 |
| **`check_status`** | 【追加】アライメント（絶対値または対立軸比率%）を判定し自動分岐する。 |
| **`check_delivery`** | 【追加】指定された単一または複数のアイテムを所持しているか判定し、**消費（没収）**して進む（複数時はアトミックに消費）。 |
| **`check_possession`** | 【追加】指定アイテムや固有フラグを判定するが、**消費・変更しない**。 |
| **`join` / `leave`** | 【追加】ゲストNPCのパーティ加入・離脱。(`is_escort_target`: true で死亡時即失敗) |
| **`trap` / `modify_state`**| 【追加】テキストと共に、HP割合低下や手札制限などの環境デバフをクエストステートに直接課す。 |
| **`modify_flag`** | 【追加】クエスト進行中のみ有効な内部変数（呪いのカウントや正解数など）を加算/減算する。 |
| **`modify_reputation`**| 【追加】道中の選択やイベントにより、名声（アライメント）を即座に増減させる。 |
| **`meet_player`** | 【追加】同じ拠点にいる他のプレイヤーをランダムに取得し、名前を一時フラグ（`met_player_name`）に保持する。 |

### 4.2 前景画像表示 (fg_image) と動的テキスト置換
- **`fg_image` (前景画像表示)**: 各ノードで背景画像の手前にキャラクターの立ち絵や宝箱のイラストなどのスプライト画像を重ねて表示する。パラメータ `fg_image` または `params.fg_image` にアセットキー（例: `"fg_chest_locked"`, `"fg_demon_soldier"`) を指定する。
- **動的テキスト置換 `{met_player_name}`**: `meet_player` ノードで取得した他のプレイヤーの名前を、テキスト内の `{met_player_name}` プレースホルダーに動的に置換して表示する。

### 4.3 チェックポイントセーブ (camp ノードの機能拡張)
`camp` タイプノードの開始時に自動的に API `/api/quest/checkpoint` がコールされ、そこまでに獲得した `lootPool` のアイテムやゴールドをDBに永続化する。このセーブ処理により、野営地から即座に獲得したアイテムを装備することが可能となる。
また、`camp` ノードで「探索を終えて帰還する（クリア）」を選択した場合、クリア用リザルト遷移準備状態（`endReady = { result: 'success' }`）に移行し、「結果を確認する」ボタンを最優先で表示する。この際、「休憩を終えて出発する」や「次へ」ボタンは非表示となる。


### 4.4 クライアント状態管理とデータ整合性
- **一時状態のクリア処理**: `camp` チェックポイントでの保存成功時、クライアント側の `lootPool`, `consumedItems`, `reputationChanges` がクリアされる。Zustandストアの状態を非同期にクリアする際、ストアの反応性を維持するため、直接の書き換えではなく `useQuestState.setState({ reputationChanges: {} })` のように `setState` アクションを通じて行う。
- **一時文字列フラグの代入保護**: `questFlags` ストアに文字列データ（`met_player_name` など）を格納する際、初期値（0）への加算（文字列結合バグ）を回避するため、`setFlag` 呼び出し時は第3引数に `isSet: true` を渡すことで直接代入として処理する。また、UI表示置換時は常に `String(metPlayerName)` としてキャストし、型安全な置換を行う。

---

## 5. 裏切りシステム (Betrayal Mechanic) と特使アイテム

### 5.1 基本仕様
クエスト中に取得または要求された「キーアイテム・消耗品」などを、クエストクリア前にショップに売却すると **「裏切り」** と判定される。
*   **ペナルティ**: 警告の後、承諾するとクエストは即座に失敗扱いとなり、名声の低下やクエスト失敗ペナルティが課される。

### 5.2 スポットシナリオ専用スキルの例外（売却不可・裏切り免除）
「四神の宝珠」や「五英霊の誓約」など、**クエストのボス攻略のためにデッキに装備して発動する固有アイテム（スキル）**は以下の例外扱いとし、裏切りの対象としない。
*   データベース上は `is_skill = true`, `type = 'skill'` として定義される。
*   システム的に**「ショップ売却不可」の専属スキル**として扱うため、プレイヤーが誤って売却してペナルティを受ける事態を防ぐ。

---

## 6. 背景画像システム (Background Image System)

### 6.1 概要
各シナリオノードは `bg_key` フィールドにより背景画像を指定する。`bg_key` は `src/config/assets.ts` の `SCENARIO_ASSETS` マップを通じて実ファイルパスに解決される。

### 6.2 bg_key → ファイルパスの管理
- **定義元**: `src/config/assets.ts` の `SCENARIO_ASSETS` オブジェクト
- **画像格納先**: `public/images/quests/` 配下
- **仕様書との整合性**: `docs/quest/quest_60*.md` の各ノードに記載された `背景画像` フィールドが正となる

### 6.3 CSVでの指定方法
CSVの `params` カラム内のJSON: `{"type":"text", "bg":"bg_wasteland"}`
- `bg` キーの値が `bg_key` としてSQLに変換される
- 1シナリオ内で複数の背景を使い分け可能（ノードごとに指定）

### 6.4 バトル時の背景
- **クエストバトル**: シナリオ側のバトルノード近傍の `bg_key` を `BattleView` の `bgImageUrl` prop に渡す
- **フリーバトル**: `bg_default` にフォールバック
- `ScenarioEngine.onBattleStart(enemyId, successNodeId, bgKey?)` → `QuestPage.startBattle()` → `getAssetUrl(bgKey)` → `BattleView.bgImageUrl`

### 6.5 話者アイコン
- テキストダイアログ内にアイコンを表示（`speaker_image_url` パラメータで指定）
- 背景レイヤーでの二重表示は行わない（v11.3で廃止）

---

## 7. パーティメンバーUI (Quest Header Party Display)

### 7.1 表示要件
- QuestHeader内にパーティメンバーアイコン一覧を表示（折りたたみUI）
- アイコンタップでステータス詳細ポップアップ（HP, ATK, DEF, スキル一覧）
- ポップアップは `ReactDOM.createPortal` で `document.body` に直接描画（`overflow:hidden` の祖先要素を回避）
- z-index: `9999`（他のUIの最前面）

### 7.2 BattleViewとの統一
- BattleView内のメンバーポップアップと同一のレイアウト・情報量を維持
- skill_names / skills / abilities 配列からスキルをピル形式で表示

---

## 8. クエスト完了処理 & 履歴管理 (Quest Complete)
1. **加齢・成長処理**: 経過日数に基づく年齢計算。
2. **報酬付与**: `quest.rewards` の金額・EXP・アイテム・最終名声変動を適用。
3. **クリア履歴の保存**: `user_completed_quests` テーブルに記録。
    - **normalクエスト**: クリア記録は保存するが、依頼一覧からの非表示には使用しない（リピート可能）。
    - **specialクエスト**: 現世代で非表示。継承時にクリア記録が削除されるため再出現する。
    - **メインシナリオ (main_ep*)**: 継承時もクリア記録を保持（永久非表示）。
4. **ステートリセット**: ゲストNPCの離脱・内部フラグ（呪い・正解数など）の完全破棄。
5. **Vitality摩耗**: パーティメンバーのVIT減少処理。
6. **ゲストNPC→通常雇用変換** (v15.2): クエスト成功時に`leave`ノードを通過せず残留したゲストNPCを`party_members`テーブルにINSERTし、正規メンバーとして永続化。パーティ上限(4名)・重複チェック付き。

### 8.0.1.1 クエスト途中（loot_pool）で獲得したゴールド・経験値・アライメントの合算反映 (v29.1)
クエスト中の選択肢や中間イベント、`reward` ノード等で増減したゴールド、経験値、アライメント値（Order / Chaos / Justice / Evil）は一時的に `loot_pool` に `{ itemId: 'gold' | 'exp' | 'align_order' | 'align_chaos' | 'align_justice' | 'align_evil', quantity: ... }` として蓄積される。クエスト完了 API（`api/quest/complete`）では、アイテム whitelist バリデーション処理（`getQuestAllowedItems`）の前にこれらを抽出し、クエスト全体の最終報酬に合算して反映する（ゴールドは `increment_gold` RPCに合算、経験値は `earnedExp` に加算、アライメント値は `effectiveRewards.alignment_shift` にマージされて最終的にプレイヤープロファイルおよび拠点アライメントに反映される）。


### 8.0.1 クエスト放棄ペナルティ (v27)

クエスト放棄（`POST /api/quest/give-up`）にはペナルティが課される。

| ペナルティ | 値 | 備考 |
|---|---|---|
| 名声減少 | -5〜-10（ランダム） | クエスト失敗と同等の重みを持たせる |
| VIT消費 | -1 | 時間浪費の代償 |

### 8.0.2 クエスト完了APIリファクタリング構造 (v27)
`quest/complete/route.ts` は以下のヘルパーモジュールに分割されている:

| ヘルパー | ファイル | 担当 |
|---|---|---|
| `applyAlignmentShift` | `services/questCompleteHelpers.ts` | アライメントpt更新 |
| `updateHubAndWorldState` | 同上 | Hub/世界情勢スコア反映 |
| `grantRewardItems` | 同上 | アイテム・スキル付与 |
| `convertGuestToPartyMember` | 同上 | ゲストNPC正規雇用 |
| `processPartyWearCycle` | 同上 | パーティVIT摩耗+形見 |
| `processReputationChange` | 同上 | 名声変動 |
| `persistLootPool` | 同上 | 戦利品保存 |

### 8.0.3 世界情勢リセットのCron分離 (v27)
世界情勢アライメントスコアの6時間リセットロジックは `services/worldStateReset.ts` に分離。

- **Cronエンドポイント**: `GET /api/cron/world-reset`（`CRON_SECRET`ヘッダーで保護）
- **フォールバック**: `location/quests` APIでもリセットモジュールを呼び出し（Cron未実行時の安全策）
- **設計原則**: 読み取りAPIに副作用を持たせない。副作用はCron/専用APIに分離する。

### 8.1 Vitality (VIT) 摩耗ルール
パーティメンバーはクエスト完了時にVitalityが減少する。VITが0になるとパーティから離脱する。

| 条件 | VIT減少量 | 備考 |
|------|-----------|------|
| クエスト成功 | -5 | 全メンバーに適用 |
| クエスト失敗・撤退 | -10 | 全メンバーに適用 |
| バトルでHP0 | 追加 -10 | 該当メンバーのみ |

- VIT初期値: 100（雇用時）
- VIT最大値: 100
- VIT 0到達: パーティから離脱（party_members削除）、形見アイテム生成
- UI表記: **VIT** (略称), **Vitality** (正式名)
- DBカラム: `party_members.durability` (内部名は変更しない)

### 8.2 完了画面 (QuestResultModal) API レスポンス構造
```json
{
  "success": true,
  "quest_title": "荒野の出発",
  "days_passed": 3,
  "new_location": "uuid",
  "new_location_name": "国境の町",
  "rewards": { "exp": 80, "gold": 200, "reputation": 10 },
  "earned_exp": 80,
  "loot_saved": [],
  "share_text": "...",
  "rep_change": { "amount": 10, "location": "マルカンド" },
  "party_changes": [
    { "name": "影の傭兵", "oldDurability": 95, "newDurability": 90, "perished": false },
    { "name": "古い剣士", "oldDurability": 10, "newDurability": 0, "perished": true, "memento": "古剣の欠片" }
  ],
  "guest_conversion": {
    "name": "ヴォルグ",
    "success": true
  },
  "changes": {
    "gold_gained": 200,
    "old_age": 18, "new_age": 18,
    "aged_up": false,
    "vit_penalty": 0,
    "atk_decay": 0, "def_decay": 0,
    "level_up": {
      "level_up": true,
      "new_level": 3,
      "hp_increase": 8,
      "atk_increase": 1,
      "def_increase": 1,
      "new_max_hp": 93,
      "new_max_cost": 12
    }
  }
}
```

> **Note (v27)**: クエスト掲示板（`QuestBoardModal`）のアイテム詳細には「所要日数: 成功 X日 / 失敗 Y日」が表示される。`days_success` / `days_failure` は `location/quests` APIから返却。

### 8.3 完了画面 UI セクション構成
| セクション | 表示内容 | 条件 |
|------------|----------|------|
| ヘッダー | 完了/失敗バッジ、クエストタイトル | 常時 |
| 報酬 | ゴールド、経験値、名声変動、ドロップアイテム | 成功時 |
| 名声ペナルティ | 名声減少（拠点名付き） | 失敗時 |
| キャラクター変化 | レベルアップ（Lv/HP/ATK/DEF/コスト数値表示）、加齢、ATK/DEF減衰 | 該当変化がある場合 |
| 時間経過 | 経過日数、到着地名 | 常時 |
| パーティ状態 | メンバーVIT変化（VIT XX ▸ YY）、離脱、形見獲得 | パーティ変化がある場合 |
| ゲストNPC加入 | 「○○が正式にパーティに加入！」または加入見送り理由 | guest_conversionがある場合 |
| シェア | SNS投稿ボタン | share_textがある場合 |

---

## 9. クエストシナリオのボリュームおよび文字数制限規則 (Scenario Size & Length Regulations) (v29)

プレイ感の担保およびUI表示の美しさを両立させるため、クエストシナリオの作成時には以下のボリューム・文字数制限規則を厳守すること。

### 9.1 クエストあたりのボリューム規定
- **ノード数**:
  - **最低制限**: 1クエストあたり **最低30ノード以上** (戦闘ノードを除いたテキスト進行部分を含む)
  - **推奨基準**: **推奨50ノード以上**
  - **最大制限**: **最大100ノード以下**
- **総テキスト量**:
  - **最低制限**: クエスト全体の総テキスト文字数は **最低800文字以上**
  - **推奨基準**: クエスト全体の総テキスト文字数は **推奨1,500文字以上**
  - ※ 単純な挨拶や短い応答の繰り返しでノード数を水増しすることは禁止する。世界観やキャラクター背景の描写を含めてテキスト量を確保すること。
  - ※ 会話のみの進行に偏らず、情景描写やプレイヤー（主人公）の独白（心理描写）等をしっかりと記述すること。これらを適度に挿入することで、旅の質感や情景を際立たせ、シナリオのプレイ感（読み応え）を担保する。

### 9.2 通常テキストノードの最大文字数制限 (Dynamic Chars Limit)
メッセージウィンドウのレイアウトおよび可読性を保つため、ノード内のテキスト文字数は以下の制限を設ける。
- **話者ありノード（発言・セリフ等）**: **最大40文字**
- **話者なしノード（地文・ナレーション等）**: **最大50文字**

### 9.3 シナリオの世界観・トーン規定
- **世界観テイスト**:
  - クラシックなダークファンタジーおよびレトロJRPG（Wizardry, Wirth, ソード・ワールド等の系譜）にインスパイアされた、硬派で重厚な世界観。
  - キャラクターの有限の命（加齢と老衰）、死と継承（英霊・残影）のシステムと密接にリンクしたストーリーテリング。
- **四大国家の文化背景の対立**:
  - **ローラン聖王国（秩序・Order）**: 騎士道と教会。裏にある異端審問や犠牲の重い質感。
  - **砂塵の王国マルカンド（混沌・Chaos）**: 自由競争と錬金術。密輸、闇競売、無法者などの退廃的なリアリズム。
  - **夜刀神国（正義・Justice）**: 神道呪術、侍・忍者・妖怪。義理と人情、怪異との調和や対峙。
  - **華龍神朝（悪・Evil）**: 覇道と実利。重税による一揆の鎮圧、悪徳官僚の護衛など、力と実利の優先。
- **文体とトーン**:
  - 現実的かつ残酷な状況を淡々と描く、シリアスで乾いた筆致。
  - 成功パスであっても単純なハッピーエンドに終わらず、ビターで余韻を残す文体を徹底する。


