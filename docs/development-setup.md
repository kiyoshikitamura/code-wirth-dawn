...
- **本番 Deploy**: 本番データを直接集計（`DASHBOARD_*` 変数不要）
- **Preview Deploy**: `DASHBOARD_SUPABASE_URL` + `DASHBOARD_SUPABASE_SERVICE_ROLE_KEY` で本番データを参照
- **ローカル**: `.env.local` の `DASHBOARD_*` 変数で本番データを参照

### デバッグメニュー (`DebugPanelGate`)
- **本番**: `localStorage` の `adminKey`（16文字以上）を持つユーザーのみ表示
- **開発/ローカル**: 全ユーザーに常時表示
...
`setup_v2.sql` はベーステーブルのみ含むため、新規 DB には **本番スキーマダンプ → 適用** のアプローチを使用：

```bash
# 1. 本番からスキーマダンプ
npx supabase link --project-ref zvoroixjuypnintkpmux
npx supabase db dump --linked -f supabase/production_schema.sql --schema public

# 2. 対象DBにリンク切替 → スキーマ適用
npx supabase link --project-ref <対象プロジェクト>
npx supabase db query --linked "DROP SCHEMA public CASCADE; CREATE SCHEMA public; GRANT USAGE ON SCHEMA public TO anon, authenticated, service_role; GRANT ALL ON SCHEMA public TO postgres;"
npx supabase db query --linked -f supabase/production_schema.sql

# 3. GRANT 付与 + PostgREST キャッシュリロード
npx supabase db query --linked "GRANT ALL ON ALL TABLES IN SCHEMA public TO anon, authenticated, service_role; GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO anon, authenticated, service_role; NOTIFY pgrst, 'reload schema';"
...
-- PostgREST キャッシュリロード（テーブル作成/カラム変更後）
