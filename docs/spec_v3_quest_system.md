...
- **目的**: クエスト受注に対するリスクの導入と、名声管理の重要性向上。

### 5.6 重度ペナルティ (UGC/Economy Defense) (Revised v16)
- **取引停止・出禁 (名声 0 未満)**:
  - 対象拠点において、「ショップでの売買」「酒場での残影雇用」「宿屋での休息」がブロックされる。
  - **救済措置 (v16)**: 「闇市」での名声ロンダリングにより 0 にリセット可能。
- **賞金稼ぎの襲撃 (名声 -100 以下)**:
  - 移動時に確定で発生。敗北時は所持金の 50% を没収され、出発地へ強制送還。詳細は [spec_v16](spec_v16_economy_reputation.md) 参照。
...
### 7.1 名声によるフィルタリング
`scenarios` テーブルに以下のカラムを追加し、受注可能なプレイヤーを制限する。
- **`max_reputation`**: この値を超える名声を持つプレイヤーは受注不可（悪人向けクエスト用）。
- **`min_reputation`**: この値を下回る名声を持つプレイヤーは受注不可（英雄向けクエスト用）。
