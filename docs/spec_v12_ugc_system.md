...
## 1.1 API認証セキュリティ要件 (v12.2)
<!-- v12.2: セキュリティレビューにより追加 -->
すべてのUGC関連APIエンドポイントは、以下の認証フローに従う。

### 認証フロー
1. **JWT必須**: `Authorization: Bearer <token>` ヘッダーのJWTトークンからSupabase `auth.getUser()` でユーザーIDを取得する。トークンが無効または未指定の場合は `401 Unauthorized` を返す。
2. **bodyのuserIdは参考値**: リクエストbodyに `userId` が含まれる場合、JWTから取得したユーザーIDと一致するか検証する。不一致の場合は `401 Unauthorized` を返す。bodyの `userId` のみでの認証は**絶対に行わない**。
3. **creator_id照合**: シナリオの更新・公開操作時は、`scenarios.creator_id` がJWT認証済みユーザーIDと一致することを確認する。

### 対象API
| エンドポイント | 認証方式 |
|---|---|
| `POST /api/ugc/save` | JWT必須 + userId一致チェック |
| `POST /api/ugc/publish` | JWT必須（`createAuthClient`経由） + userId一致チェック |
| `POST /api/ugc/archive` | JWT必須（`createAuthClient`経由） |
| `POST /api/ugc/asset/enemy` | JWT必須 |
| `POST /api/ugc/asset/item` | JWT必須 |
...
### 3.1.1 エディタUI仕様 (v12.2)
<!-- v12.2: UI/UXレビューにより追加 -->
- **モバイルファースト設計**: 390px幅のモバイルシェル内に収まる1カラムレイアウト。
- **テーマ**: ゲーム全体のamber/slateダークテーマに統一（旧仕様の紫テーマは廃止）。
- **タブナビゲーション**: 4タブ構成（基本情報 / ノード / 報酬 / アクション）。
- **ノードUI**: アコーディオン方式で展開/折りたたみ可能。ノードタイプは色分け表示（会話=amber, バトル=red, 納品=green, NPC=blue）。
- **ローディング画面**: ゲーム共通のアニメーションアイコン+テキスト形式に統一。
...
- **アイテム / スキルカードの作成制限**:
  ...
  - **コスト算定**: カードの強さを `power_score = effect_value / max(ap_cost, 1)` で数値化し、`cost_val = clamp(round(power_score × 2), 1, 20)` として自動算出する。効果量が大きいのにAPコストが低い（壊れ性能）場合、cost_valは自動的に上限（20）に跳ね上がる。（※v28: 旧VIT/MPコストとしてのcost_valは廃止済。UGCではデッキバランス用の重み付け値として使用。）
