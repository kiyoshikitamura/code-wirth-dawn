# カード（スキル）マスタ仕様定義書 (v4.5)

本仕様書は、「Code: Wirth-Dawn」のバトルロジックおよび経済システムの基盤となる、プレイヤブルおよびNPC向けのスキル（カード）マスタデータの設計ドキュメントです。

> **v4.5 改定内容**: 全既存スキルおよび魔術学院追加スキルへのレアリティ（SR / R / U / C）分類の導入。共通マッピング（`src/lib/rarity.ts`）の実装、魔術学院ベーシックパック「黎明の知識と古の契約」の実装およびパック内カードリスト確認UIでの活用について追記。
> **v4.4 改定内容**: 即死無効化（`death_immune`）仕様の反映。即死攻撃（ID 58）および死神の宣告（ID 121）が対象エネミーの即死耐性に対応し、耐性持ち（ボス等）には即死が無効化され大ダメージやフォールバックダメージが適用されるように改修。
...
また、スキル解放・インベントリ・魔術学院等での管理のため、ゲーム内のすべてのスキル（`skills`）およびカードに対して、論理的な**レアリティ（SR / R / U / C）**を設定します。これはDBの物理スキーマ変更を避けるため、共通モジュール `src/lib/rarity.ts` の定数マッピングによって制御されます。
�びプログラム実装と整合。
> **v4.2 改定内容**: バトルエンジンv4.2対応。未使用スキル「瞑想 (`card_meditation` / ID 64)」の実装、および新規状態異常（AP回復効果） `ap_recover` の追加を反映。
> **v4.1 改定内容**: バトルエンジンv4.1対応。即死防止のためのバフ効果（`atk_up`, `def_up`, `evasion_up` 等）重複加算仕様の実装。裁きのダメージ値を50に調整。
> **v4.0 改定内容**: バトルエンジンv4.0対応。魔導書カード3種(火球/氷槍/雷撃)追加、即死攻撃失敗時フォールバックダメージ、吸血(drain)HP回復実装、王の城壁DEF+50修正、五星の加護攻撃+バフ化、デバフ付与成功率テーブル追加。ダメージ計算は揺らぎ(±15%)→クリティカル(×1.5)→DEF減算の順。
...
| `ap_recover` | AP回復 | プレイヤー | 即時：APを効果量分（瞑想は3）回復する。最大値（15）を超えない |
...
| 11 | `card_meditation` | 瞑想 | Support | 0 | self | 3 | ap_recover | 600 | 心を落ち着かせて瞑想し、APを3回復する（AP消費0で自身対象） |
...
| **v4.5** | **2026-06-24** | **魔術学院ベーシックパック「黎明の知識と古の契約」の追加、全既存スキル（クエスト報酬等を含む）へのレアリティ分類定義マッピングの導入。** |

---

## 8. 全スキル レアリティ（Rarity）論理定義マッピング

ゲーム内に存在するすべてのスキル（カード）に対して、共通定義 `src/lib/rarity.ts` にて以下の通りレアリティをマッピングする。これらは魔術学院のカードリスト確認UIおよびパックの排出制御に利用される。

### 💎 スーパーレア (SR) - 23種
- **地方・首都の高位奥義スキル (12種)**:
  - `3021` (聖剣), `3022` (裁き), `3035` (居合切り), `3041` (王の城壁), `3042` (ドラゴンダイブ), `3043` (奇跡), `3045` (絶対防御), `3046` (百裂拳), `3047` (メテオストライク), `3048` (完全治癒), `3049` (ホーリーノヴァ), `3050` (暗殺), `3051` (魔刃), `3053` (獅子の心), `3055` (岩砕き), `3070` (天翔斬), `3078` (虚空撃), `3079` (女神の祝福), `3080` (死の舞踊)
- **クエスト・ボス戦利品スキル (11種)**:
  - `3071` (五星の加護), `3072` (冥食の理), `3073` (神殺しの光芒), `3074` (砂塵の支配), `3081` (獄炎 of 刻印), `3082` (天使の恩寵), `3083` (竜炎のブレス), `3084` (麒麟の結界), `3085` (ゴーレムコア), `3086` (海神の怒涛), `3087` (覇王の大斧)
- **闇市最上位禁書 (4種)**:
  - `3058` (即死攻撃), `3060` (魂の生贄), `3075` (黒曜球), `3088` (時止めの法)

### 👑 レア (R) - 15種
- **中位・高位戦闘スキル (11種)**:
  - `3023` (祈り), `3024` (治癒), `3025` (聖壁), `3029` (蜃気楼), `3031` (ツバメ返し), `3037` (龍の咆哮), `3039` (連撃), `3044` (皇帝の盾), `3054` (疾風術), `3068` (魂裂き), `3069` (幻影撃), `3076` (近衛の盾), `3077` (竜の鱗)
- **闇市中位禁書 (4種)**:
  - `3056` (吸血), `3057` (闇の代償), `3059` (狂戦士の薬), `3052` (血の怒り)

### ✨ アンコモン (U) - 15種
- **地方中位・汎用中位スキル (15種)**:
  - `3027` (砂塵の目眩まし), `3028` (毒刃), `3030` (オアシスの水), `3032` (クナイ投げ), `3033` (影縫い), `3034` (清め), `3036` (気の癒やし), `3038` (鉄布衫), `3040` (飛刀), `3062` (血の契約書), `3065` (火球), `3066` (氷槍), `3067` (雷撃), `3141` (魔界の黒炎), `3142` (シャドークロウ)

### 🛡️ コモン (C) - 13種
- **初期・汎用基礎スキル (13種)**:
  - `3011` (強打), `3012` (斬撃), `3013` (突き), `3014` (防御), `3015` (応急手当), `3016` (シールドバッシュ), `3017` (集中), `3018` (クイックステップ), `3019` (挑発), `3020` (石投げ), `3026` (砂の罠), `3061` (調毒), `3063` (瞑想)
