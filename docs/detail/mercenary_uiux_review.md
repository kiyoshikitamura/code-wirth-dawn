...
## 4. v12.0 実施済み改修 (2026-04-09)

### 4.1 UI改修（`TavernModal.tsx`）
- **一覧UI刷新**: 縦4段 → 2行コンパクトレイアウトに変更
  - 1行目: アイコン + 称号+名前 + バッジ + 契約金
  - 2行目: Lv・職業（日本語）+ スキルタグ（最大4枚）
- **種別バッジ新設**:
  - `shadow_active` → 「残影」バッジ（青）
  - `shadow_heroic` → 「英霊」バッジ（金）
  - `system_mercenary` → バッジなし（「公式」バッジは削除）
- **詳細ポップアップのステータス順**: 攻撃→防御→HP → **HP→攻撃→防御** に修正
- **「影の記録」タブ**: 「準備中」プレースホルダー → 自分の英霊ダッシュボードとして実装
- **「冒険者を探す」ボタン**: セカンダリボタンスタイルを「休息する」と同一の琥珀色に統一

### 4.2 サービス改修（`shadowService.ts`）
- **スキル表示バグ修正**: アクティブ残影の`signature_deck`カードIDを、cardsテーブルから一括名前解決
- **NPC画像フォールバック**: DBに`image_url`がない場合、`/images/npcs/{slug}.png`を自動生成
- **称号フィールド追加**: `ShadowSummary`インターフェースに`epithet`フィールドを追加

### 4.3 料金変更（`game_rules.ts`）
- `HIRE_ACTIVE_PER_LEVEL`: 100G → **1,000G**（アクティブ残影の雇用費用をLv×1000Gに引き上げ）

### 4.4 新規API（`/api/tavern/my-heroic`）
- `GET /api/tavern/my-heroic?user_id={id}`
- `historical_logs`から自分の英霊レコードを取得
- サブスクリプションtierと英霊登録上限も返却
