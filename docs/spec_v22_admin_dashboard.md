...
### 2.3 統計データテーブル・セクション
管理者ダッシュボードはグラフ表現（SVGグラフ等）を廃止し、すべてJST（日本標準時）を基準とした数値テーブルで表示します。
UIは2階層 of タブナビゲーションで構成されます。

1. **親タブ (期間軸)**:
   - **「日次統計」**: 過去指定日数分（30日/60日/90日）の日別データを表示。
     - **期間選択セレクトボックス**: 日次統計のアクティブ時に表示され、表示する日数を「30日間（デフォルト） / 60日間 / 90日間」から選択できます。
   - **「月次統計」**: 過去1年間の月別データを表示。
2. **子タブ (機能軸)**: 各親タブ（日次／月次）の配下に以下の3つの子タブを持ちます。
   - **「基本KPI」**:
     - **日次カラム**: 日付、課金額、DAU、PU、PUR（課金率 % = PU/DAU*100）、ARPPU（売上/PU）、ARPU（売上/DAU）、登録ユーザー数合計（本登録数/ゲスト数の内訳）
     - **月次カラム**: 対象月（YYYY/MM）、課金額、MAU、MPU、MPUR（課金率 % = MPU/MAU*100）、MARPPU（売上/MPU）、MARPU（売上/MAU）、登録ユーザー数合計（本登録数/ゲスト数の内訳）
   - **「コロシアム」**:
     - コロシアム総合サマリー（累計プレイヤー数、総挑戦数、制覇率、最大連勝、累計回収ゴールド）を上部に表示。
     - **日次／月次カラム**: 日付/対象月、挑戦数（Easy/Normal/Hard内訳）、制覇数（同内訳）、放棄数（同内訳）、回収ゴールド
   - **「魔術学院」**:
     - 魔術学院総合サマリー（累計購入プレイヤー数、総パック購入数、累計回収ゴールド、累計返還ゴールド）を上部に表示。
     - **日次／月次カラム**: 日付/対象月、パック開封数（シリーズ別内訳）、消費ゴールド、返還ゴールド、実質消費ゴールド（消費ゴールド - 返還ゴールド）

* **CSV出力ボタン**: 各テーブルの上部に配置され、現在表示中のテーブル内容（日次または月次）をCSVファイルで出力します（Excelの文字化けを防ぐため、先頭にBOM `0xEF, 0xBB, 0xBF` を付与します）。
...
* **ヘッダー制限**:
  * `x-admin-key` の照合が必須。
* **主要処理ロジック**:
  1. `user_profiles` から全ユーザーのアクティブ判定（`updated_at`）、サブスクプラン、所持ゴールド、レベル、および登録日（JST）・匿名フラグを集計。
  2. `battle_sessions` から日別・月別の戦闘回数および勝敗数を集計。
  3. `quest_activity_logs` および `colosseum_activity_logs`、`payment_logs` などの各種行動・決済ログから、日別・月別のアクティブUUおよび課金UUを集計。
  4. 日次集計データ（`dailyKPI`）の作成: 過去 `days` 日間の各日付に対し、課金額、DAU、PU、新規登録数合計（本登録数／ゲスト数内訳）をマッピング。
  5. 月次集計データ（`monthlyKPI`）の作成: 過去1年間（当月含む12ヶ月）の月単位で、課金額、MAU（当月に1回以上アクションを行ったユニーク数）、MPU（当月に1回以上決済を行ったユニーク数）、新規登録数合計（本登録数／ゲスト数内訳）をマッピング。
  6. コロシアム（`colosseum`）および魔術学院（`academy`）についても、日次（`daily`）と月次（`monthly`）の集計データを算出して返却。

* **月間アクティブ (MAU/MPU) の算出**:
  日次のスライディングウィンドウ（過去30日間のユニークユーザー数）とは別に、**月次統計におけるMAU/MPU** は「カレンダー月（1日〜末日）におけるユニークユーザー数」として算出します。
  各日付/月において、JSTタイムゾーン基準で作成されたユーザーのアクティビティ（バトルセッション、クエスト開始、コロシアム開始など）のユニークユーザーIDセットを元にカウントします。
  ```typescript
  // 月別（YYYY/MM）のアクティブユーザー集合マップからユニーク数を集計する例
  const mau = activeUsersByMonth[monthStr] ? activeUsersByMonth[monthStr].size : 0;
  ```
