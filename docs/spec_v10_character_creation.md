...
<!-- v27.0: avatarアップロードJWT認証統一（x-user-id廃止）、法的ページ4種追加（/legal/terms,privacy,tokusho,credits）、タイトル画面フッター追加、DeleteConfirmModal共通化、外部テクスチャURLローカル化、rootリダイレクト判定をprofile.idに修正 -->
...
| パス | ページ名 | 説明 |
|---|---|---|
| `/legal/terms` | 利用規約 | サービス利用条件、禁止事項、免責事項 |
| `/legal/privacy` | プライバシーポリシー | 収集情報、利用目的、第三者提供、外部サービス |
| `/legal/tokusho` | 特定商取引法に基づく表記 | 事業者情報、返金ポリシー、決済方法 |
| `/legal/credits` | 権利表記 / Credits | OSS、フォント、BGM、画像素材のライセンス |

### 7.2 共通レイアウト
- `src/app/legal/layout.tsx` で4ページ共通のヘッダー・フッター・ナビゲーションを提供
- ヘッダー: タイトル画面への戻りリンク
- フッター: 4ページ間の相互リンク + コピーライト

### 7.3 タイトル画面フッター
- MENU画面下部に法的ページリンク（利用規約/プライバシー/特商法表記/権利表記）+ コピーライト表記を表示
- 10px/9pxテキストで控えめに配置

### 7.4 注意事項
- テンプレート内の `[事業者名]` `[メールアドレス]` `[管轄裁判所名]` 等のプレースホルダーは正式運用前に実際の情報に置換すること
- 課金機能（spec_v13）実装前に法的レビューを受けること