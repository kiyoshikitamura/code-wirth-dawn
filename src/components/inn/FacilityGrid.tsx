...
export default function FacilityGrid({ onSelectFacility, isHub = false, recommendedFacility = null, isTourActive = false }: FacilityGridProps) {
            ...
            {facilities.map((item) => {
                ...
                return (
                    <button
                        ...
                        disabled={isDisabled}
                        className={`flex items-center gap-3 p-3 md:p-4 bg-[#122042]/80 border rounded-xl transition-all text-left group shadow-lg shadow-[#0a1628]/50 focus:outline-none focus:ring-1 focus:ring-amber-400/50 ${
                            isDisabled
                                ? 'opacity-30 border-[#2a4080]/20 pointer-events-none'
                                : 'active:scale-95 hover:bg-[#1a2d5a]'
                        } ${
                            isRecommended
                                ? 'border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.4)] animate-pulse'
                                : !isDisabled ? 'border-[#2a4080]/50 hover:border-amber-500/30' : ''
                        }`}
                    >
