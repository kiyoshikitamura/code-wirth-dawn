...
export default function HistoryArchiveModal({ userId, onClose }: HistoryArchiveModalProps) {
    ...
    const [data, setData] = useState<{
        ...
        world_history: any[];
        lineage: any[];
        banned_locations: { location_name: string; score: number }[];
    }>({ chronicle: [], world_history: [], lineage: [], banned_locations: [] });
                        ...
                        <div className="space-y-4 animate-in fade-in duration-500">
                            ...
                            {activeTab === 'personal' && (
                                <div className="space-y-6">
                                                    ...
                                                    <div className="bg-white/5 border border-white/10 p-4 rounded-lg hover:border-amber-500/50 transition-colors">
                                                        <div className="flex justify-between items-start mb-2">
                                                            <div className="text-amber-500 text-xs font-bold tracking-tighter flex items-center gap-1">
                                                                ...
                                                                {item.accumulated_days_at_completion != null
                                                                    ? (() => {
                                                                        const d = item.accumulated_days_at_completion;
                                                        ...
                                                        <div className="flex items-center gap-3 text-xs text-gray-400">
                                                            <div className="flex items-center gap-1 bg-black/30 px-2 py-0.5 rounded">
                                                                ...
                                                                <span>{item.scenarios.locations?.name || '発生地不明'}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    {/* フレーバー統計: 出禁拠点 */}
                                    {data.banned_locations.length > 0 && (
                                        <div className="mt-6 border border-red-900/40 rounded-lg bg-red-950/10 p-4">
                                            <h4 className="text-xs font-bold text-red-400 tracking-widest uppercase mb-3 flex items-center gap-2">
                                                <Skull className="w-3 h-3" />
                                                出入り禁止の拠点
                                            </h4>
                                            <div className="space-y-2">
                                                {data.banned_locations.map((loc, i) => (
                                                    <div key={i} className="flex items-center justify-between bg-black/30 rounded px-3 py-2">
                                                        <div className="flex items-center gap-2 text-xs text-gray-400">
                                                            <MapPin className="w-3 h-3 text-red-500" />
                                                            <span>{loc.location_name}</span>
                                                        </div>
                                                        <span className="text-[10px] font-mono text-red-500 font-bold">{loc.score}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
